...
Scala Twirl templates are closer to .NET Razor templates then to Java Server Faces. The templates are compiled to Scala object and therefore type-safe with the support of all the language features. On the other hand, they should handle only simple tasks and all the business logic is placed in the controller actions.

I continued to showcase the implementation of prototype 1.0 functionality again in the context of the pipeline workflow. The last section of the implementation chapter was dedicated to the application deployment in a lot of detail. The reason for such a detailed description was to document the process for a future reference. Overall, I have quite positive experience using Azure cloud services.
